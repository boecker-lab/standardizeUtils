    ...
    def standardize_structures(self):
        ...
        with ccf.ProcessPoolExecutor(max_workers=number_of_processors) as executor:
            ...
            for idx in range(len(self.__input_structures)):
                ...
                processing_queue[idx] = executor.submit(standardize_structure_with_pubchem, input_structure=self.__input_structures[idx], input_format=self.__input_format, output_format=self.__output_format, request_processing_time=self.__request_processing_time)
...
def standardize_structure_with_pubchem(input_structure, input_format, output_format=None, return_none_if_error=True, request_processing_time=30):
    try:
        ...
        if request_response.get_status() == 'FINISHED':
            return request_response.get_standardized_structure()
        else:
            ...
            return response_handler.await_result(sec=request_processing_time)
...
def standardize_smiles_with_rdkit(smiles, isomeric_smiles=False, kekule_smiles=False, rooted_at_atom=-1, canonical=True, all_bonds_explicit=False, all_hydrogen_explicit=False):
    mol = Chem.MolFromSmiles(smiles)
    return Chem.MolToSmiles(mol, isomericSmiles=isomeric_smiles, kekuleSmiles=kekule_smiles, rootedAtAtom=rooted_at_atom, canonical=canonical, allBondsExplicit=all_bonds_explicit, allHsExplicit=all_hydrogen_explicit)


def standardize_inchi_with_rdkit(inchi, remove_hydrogens=True, remove_stereochemistry=True):
    mol = Chem.inchi.MolFromInchi(inchi=inchi, removeHs=remove_hydrogens)
    if remove_stereochemistry:
        Chem.RemoveStereochemistry(mol)
    return Chem.inchi.MolToInchi(mol)


def standardize_inchi_list_with_rdkit(inchi_iterable, remove_hydrogens=True, remove_stereochemistry=True):
    standardized_inchi_list = []
    for inchi in inchi_iterable:
        standardized_inchi = standardize_inchi_with_rdkit(inchi=inchi, remove_hydrogens=remove_hydrogens, remove_stereochemistry=remove_stereochemistry)
        standardized_inchi_list.append(standardized_inchi)
    return standardized_inchi_list


def standardize_smiles_list_with_rdkit(smiles_iterable, isomeric_smiles=False, kekule_smiles=False, canonical=True, all_bonds_explicit=False, all_hydrogen_explicit=False):
    standardized_smiles_list = []
    for smiles in smiles_iterable:
        standardized_smiles = standardize_smiles_with_rdkit(smiles, isomeric_smiles=isomeric_smiles, kekule_smiles=kekule_smiles, canonical=canonical, all_bonds_explicit=all_bonds_explicit, all_hydrogen_explicit=all_hydrogen_explicit)
